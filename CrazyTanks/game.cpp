				...
				if (r + 1 == kLevelRows && c + 1 == kLevelColumns)
				{
					...
					while (n != 0)
					{
						...
						if (z == 1)
						{
							for (int q = j; q < j + l; q++)
							{
								if (kLevelData0[i][q] == GameObjectType_TankPlayer|| kLevelData0[i][q] == GameObjectType_Wall)
									o++;
							}
						...
						else
						{
							for (int q = i; q < i + l; q++)
							{
								if (kLevelData0[q][j] == GameObjectType_TankPlayer || kLevelData0[q][j] == GameObjectType_Wall)
									o++;
							}
							if (o == 0)
							{
			...
			case CellSymbol_EnemySpawner:
			{
				...
				while (k!=0)
				{
					...
					for (int z = i - 3; z < i + 3; z++)
					{
						...
						{
							if (kLevelData0[z][y] == GameObjectType_EnemySpawner)
								n++;
						}
					}
					if (n==0)
